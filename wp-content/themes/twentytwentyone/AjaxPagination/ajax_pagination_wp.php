...
function query_ajax_pagination($post_type = 'post', $cat = 19, $posts_per_page = 5, $paged = 1)
{
    ...
    if ($q_svl->have_posts()) {
        ...
        while ($q_svl->have_posts()) {
            ...
            $allpost .= '<div class="col-lg-6 col-md-6 col-12 mb-3">';
            ...
            $allpost .= '  <span class="text-secondary-foreground flex items-center">';
            $allpost .= '<img src="' . get_template_directory_uri() . '/assets/images/Writer.svg" alt="Image Description" class="">&nbsp; Last update ' . esc_html(get_the_date()) . ' &nbsp; <span class="text"> <span class="by-t">by</span>&nbsp; <span class="text-primary ml-1">' . get_the_author() . '</span></span>';
            $allpost .= '   </span>';
            $allpost .= '</div>';
            $allpost .= '</div>';
            $allpost .= ' </a>';
