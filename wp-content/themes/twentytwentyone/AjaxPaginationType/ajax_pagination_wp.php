...
function query_ajax_pagination_type($post_type = 'post',$cat = 22, $type="" , $posts_per_page = 1, $paged = 1)
{
    ...
    if ($q_svl->have_posts()) {  
        ...
        while ($q_svl->have_posts()) {
            ...
            $allpost .= '<img src="'. get_template_directory_uri().'/assets/images/Writer.svg" alt="Image Description" class="">';
            $allpost .= '  &nbsp; Last update '. esc_html( get_the_date() ) .' &nbsp; <span class="text"> <span class="by-t">by</span>&nbsp; <span class="text-primary ml-1">'. (!empty($artist) ? $artist : get_the_author()).'</span></span>';
            $allpost .= '  </span>';
            $allpost .= '</div>';
            $allpost .= '</div>';
            $allpost .= '</div>';
